    ...
    public void onTagDiscovered(Tag tag) {
        ...
        if (ndef != null) {
                ...
                if (ndefMessage != null) {
                    ...
                    try {
                        ...
                        if (encontrado) {
                            ...
                            // Vibra por 300 milisegundos cuando se detecta una etiqueta NFC incorrecta
                            ...
                            new Handler(Looper.getMainLooper()).postDelayed(() -> {
                                // Cambia a la nueva actividad después de 3 segundos
                            ...
                            // Rellenar la tabla de asistencia para el día de hoy (en caso de que el usuario salga del centro)
                            FillAttendanceTask fillAttendanceTask = (FillAttendanceTask) new FillAttendanceTask(CardActivity.this).execute(username);
                            fillAttendanceTask.get();

                            SharedPreferences.Editor editor = sharedPreferences.edit();
                            editor.putBoolean("avisado", false);

                        }
                        else {

                            // Crear y reproducir el sonido
                            //MediaPlayer mediaPlayer = MediaPlayer.create(this, R.raw.access_denied);
                            //mediaPlayer.start();
                            ...
                            runOnUiThread(() -> {
                                ...
                                imageViewInfo.setVisibility(View.VISIBLE);
                                textViewInfo.setText("No perteneces a esta clase.");
                                textViewInfo.setVisibility(View.VISIBLE);

                                // Obtener las LayoutParams de la ImageView
                                ConstraintLayout.LayoutParams paramsImage = (ConstraintLayout.LayoutParams) imageViewInfo.getLayoutParams();
                                // Mover la imagen hacia arriba 100dp
                                paramsImage.topMargin = paramsImage.topMargin - (int) TypedValue.applyDimension(TypedValue.COMPLEX_UNIT_DIP, 100, getResources().getDisplayMetrics());
                                imageViewInfo.setLayoutParams(paramsImage);

                                // Obtener las LayoutParams del TextView
                                ConstraintLayout.LayoutParams paramsText = (ConstraintLayout.LayoutParams) textViewInfo.getLayoutParams();
                                // Mover el texto hacia arriba 325dp
                                paramsText.topMargin = paramsText.topMargin - (int) TypedValue.applyDimension(TypedValue.COMPLEX_UNIT_DIP, 325, getResources().getDisplayMetrics());
                                textViewInfo.setLayoutParams(paramsText);
            ...
            runOnUiThread(() -> {
                ...
                imageViewInfo.setVisibility(View.VISIBLE);
                imageViewInfo.setImageResource(R.drawable.red_cross);
                textViewInfo.setText("Etiqueta NFC no soportada.");
                textViewInfo.setVisibility(View.VISIBLE);

    ...
    protected void onPostExecute(String resultado) {
        ...
        if (exception != null) {
            ...
            for (int i = 0; i < listaItem.getLength(); i++) {
                ...
                String var_nombre_completo =  element.getElementsByTagName("nombre_completo").item(0).getTextContent();
                ...
                Session session = new Session(var_nombre, Integer.parseInt(var_num_dia_semanal), var_hora_inicio, var_hora_final);
                arrayOfSessions.add(session);
                setOfNames.add(var_nombre_completo);
            }

            // Enviar el ArrayList a la actividad principal
            ((ScheduleActivity) context).showSessionsOnScreen(arrayOfSessions, setOfNames);
        }
    }
